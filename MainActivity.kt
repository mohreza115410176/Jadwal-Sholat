package com.example.usergate.jadwalsholat
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mKotaAdapter = ArrayAdapter<Kota>(this, android.R.layout.simple_spinner_item, li
                mKotaAdapter!!.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_
                kota.adapter = mKotaAdapter
                kota.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onNothingSelected(p0: AdapterView<*>?) {
            }
            override fun onItemSelected(p0: AdapterView<*>?, view: View?, position: Int,
                                        val kota = mKotaAdapter!!.getItem(position)loadJadwal(kota.id)
        }
    }
    loadKota()
}
...
fun loadKota() {
    try {
        ...
        val task = ClientAsyncTask(this, object : ClientAsyncTask.OnPostExecuteListe
...
private fun loadJadwal(id: Int?) {
    try {
        ...
        val tanggal = current.format(Date())var url = "https://api.banghasan.com/sholat/format/json/jadwal/kota/$id_kota
        val task = ClientAsyncTask(this, object : ClientAsyncTask.OnPostExecuteListe
                override fun onPostExecute(result: String) {
